...
db().then();
const data = JSON.parse(fs.readFileSync("_collection/customers.json", {encoding: 'utf-8'}));
...
router.get("/transfer/:accountNo", async (req,res)=> {
   ...
   const accountNo = await Customers.findOne({ accountNo : req.params.accountNo});
   const name = await Customers.findOne({ accountNo : req.params.accountNo});
   if(accNo == null) res.redirect("/allcustomers");

   res.render("transfer.ejs", {name :name.name,accNo :accNo.currentBalance,accountNo:accountNo.accountNo, message:"Tranfer Money" } )
})
...
router.get("/transactionHistory", async(req,res)=> {
   const alluser = await Transfers.find({});
   try 
   {
